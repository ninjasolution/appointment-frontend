...
import { selectClientAction } from '../../../../store/actions/GlobalAction';
...
function NewSalePage() {
    ...
    const client = useSelector(s => s.globals.selectedClient)
    const products = useSelector(s => s.globals.products)
                ...
                <div className='clients-details'>
                    {
                        client?._id ?
                            <div className='client-list'>
                                ...
                                <div className='client-item' onClick={() => dispatch(selectClientAction(client))} >
                                    <div className='client-avatar'>
                                        <span>{getLogo(client?.firstName, client?.lastName)}</span>
                                    </div>
                                    <div style={{ display: "flex", justifyContent: "space-between", padding: "0px 20px 0px 10px", width: "100%" }}>

                                        <div className='client-data'>
                                            <span id='client-name'>{`${client?.firstName} ${client?.lastName}`}</span>
                                            <span id='client-email'>{client?.email}</span>
                                        </div>

                                        <div style={{ display: "flex", alignItems: "center" }}>
                                            <svg width={25} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M18 14a2 2 0 110-4 2 2 0 010 4zm-6 0a2 2 0 110-4 2 2 0 010 4zm-8-2a2 2 0 104 0 2 2 0 00-4 0z" fill="#101928"></path></svg>
                                        </div>
                                    </div>
                                </div>
                            </div>
...

                    {
                        client?._id ?
                            ...
                            <div className='client-none-state'>

                                {
                                    ...
                                    <div className='add-client-button'>
                                        ...
                                        <span onClick={() => setListViewState(true)}>Add client</span>
                                    </div>
                                }
                                ...
                                }
                                {
                                    products?.map((item, key) => (
                                        <div key={key} className='cart-item'>
                                            <div>
                                                <p className='blue-title'>{item.name}</p>
                                                <p>{`${item.skus[0]} ${item.supplier?.firstName} ${item.supplier?.lastName}`}</p>
                                            </div>
                                    ...
                                    <div className="client-list">
                                        {
                                            clients?.map((item, key) => (
                                                ...
                                                <div key={key} className='client-item' onClick={() => dispatch(selectClientAction(item))}>
                                                    <div className='client-avatar'>
                                                        <span>{getLogo(item.firstName, item.lastName)}</span>
                                                    </div>
                                                    <div className='client-data'>
